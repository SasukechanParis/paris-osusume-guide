// Renders /en/restaurants-and-cafes-paris.html from
// en/data/restaurants-cafes-en.json — Sasuke's own restaurant and café
// picks, ported from data/recommendations.json with his approval (see
// docs/en-site-plan.md, "Round 2").
//
...
async function loadItems() {
  const res = await fetch('data/restaurants-cafes-en.json');
  if (!res.ok) throw new Error(`Failed to load restaurants-cafes-en.json: ${res.status}`);
  return res.json();
}
...
async function init() {
  ...
  if (!restaurantsList || !cafesList) return;

  let all;
  try {
    all = await loadItems();
  } catch (err) {
    restaurantsList.innerHTML = '<p class="trending-desc">This list is temporarily unavailable. Please try again shortly.</p>';
    console.error(err);
    return;
  }

  const restaurants = all.filter((item) => item.category === 'restaurant');
  const cafes = all.filter((item) => item.category === 'cafe');

  restaurantsList.innerHTML = renderList(restaurants);
  cafesList.innerHTML = renderList(cafes);

  setupNearbySearchEn([...restaurants, ...cafes].map(toNearbyItem));
