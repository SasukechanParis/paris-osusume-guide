// Renders /en/chocolatiers-and-patisseries-paris.html from
// en/data/chocolatiers-patisseries-en.json — Sasuke's own chocolatier and
// pâtisserie picks, ported from data/recommendations.json with his
// approval (see docs/en-site-plan.md, "Round 2").
//
...
async function loadItems() {
  const res = await fetch('data/chocolatiers-patisseries-en.json');
  if (!res.ok) throw new Error(`Failed to load chocolatiers-patisseries-en.json: ${res.status}`);
  return res.json();
}
...
async function init() {
  const chocolatiersList = document.getElementById('chocolatiers-list');
  const patisseriesList = document.getElementById('patisseries-list');
  if (!chocolatiersList || !patisseriesList) return;

  let all;
  try {
    all = await loadItems();
  } catch (err) {
    chocolatiersList.innerHTML = '<p class="trending-desc">This list is temporarily unavailable. Please try again shortly.</p>';
    console.error(err);
    return;
  }

  const chocolatiers = all.filter((item) => item.category === 'chocolatier');
  const patisseries = all.filter((item) => item.category === 'patisserie');

  chocolatiersList.innerHTML = renderList(chocolatiers);
  patisseriesList.innerHTML = renderList(patisseries);

  setupNearbySearchEn([...chocolatiers, ...patisseries].map(toNearbyItem));
