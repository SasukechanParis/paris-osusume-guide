// Renders /en/paris-souvenir-guide.html from en/data/souvenirs-en.json —
// Sasuke's own souvenir-shop picks, ported from data/recommendations.json
// with his approval (see docs/en-site-plan.md, "Round 2").
//
...
async function loadItems() {
  const res = await fetch('data/souvenirs-en.json');
  if (!res.ok) throw new Error(`Failed to load souvenirs-en.json: ${res.status}`);
  return res.json();
}
