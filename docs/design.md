...
## 投稿機能・アクセス設計(2026-08-31 追記、方針確定)

### さすけのおすすめの配布方針

- `recommendations.html` は全ページ共通の `.site-nav` およびトップページの `category-grid` から意図的に外してある(URLを直接知っている人だけが開ける「隠しURL」)
- 撮影が成約したお客さんにだけ、むんたが個別のLINEメッセージでURLを手渡しする運用にする
- パスワード等の認証は付けない。「信用しているお客さんにしか渡さない」ため、隠しURL程度の軽い壁で十分と判断(2026-08-31 むんた確認)
- 注意点: URLはあくまで「知らなければ辿り着けない」レベル。お客さんがSNS等に転載すれば誰でも見られる状態になるため、厳密なアクセス制御ではないことを前提に運用する
- 一方、`パンコンクール`(`bread.html`)・`ミシュラン`(`michelin.html`)・`投稿する`(`post.html`)は全ページの`.site-nav`とトップページに残し、公式LINEのリッチメニュー等で誰にでも公開してよい集客用コンテンツとして扱う
