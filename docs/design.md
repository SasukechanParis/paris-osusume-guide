...
- `recommendations.html` は他ページと同様に `.site-nav` とトップページの `category-grid` に通常表示する(サイト構造としては隠さない)
...
- 検討したが不採用: 「おすすめ」だけナビから外して隠しURL化する案。むんたの運用はURL自体を直接手渡しするだけなので、サイト内で個別ページを隠す必要はないと判断
