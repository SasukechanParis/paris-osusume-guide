...
- `hotels.html` — ホテル(「先輩カップルのおすすめ」がデフォルト表示。「さすけのおすすめ」タブは意図的に空にしている。ホテルは基本一度しか泊まらず比較できないため、さすけ自身の判断でのおすすめ掲載をやめた方針。特別なホテル(Ritz Paris・Airelles Château de Versailles, Le Grand Contrôle)は別枠で紹介。近くの店検索つき)
