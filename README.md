...
- `hotels.html` — ホテル(さすけ自身のおすすめのみ掲載、先輩カップルのタブは無し。さすけは撮影で500件以上のホテルを訪れており比較ができるが、先輩カップルは一度しか宿泊できず比較できないため、という方針。特別なホテル(Ritz Paris・Airelles Château de Versailles, Le Grand Contrôle)は別枠で紹介。近くの店検索つき、`js/hotels.js`は`category-page.js`を使わない専用スクリプト)
